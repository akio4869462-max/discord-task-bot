...
def test_add_exp_triggers_level_up_when_threshold_reached():
    ...
    is_up, level_or_diff, event = study_logic.add_exp('programming', required_minutes)
    assert is_up is True
    assert level_or_diff == 2
...
def test_boss_appears_after_threshold_minutes():
    ...
    # 閾値に到達した瞬間にボス出現
    is_up, level_or_diff, event = study_logic.add_exp('programming', 1)
    assert event == 'BOSS_APPEAR'
    data = study_logic.load_player_data()
...
def test_boss_takes_damage_then_is_defeated():
    ...
    # HPより少ない作業ではダメージのみ
    is_up, level_or_diff, event = study_logic.add_exp('programming', first_boss['hp'] - 1)
    assert event == 'BOSS_DAMAGE'
    assert study_logic.load_player_data()['is_boss_active'] is True

    # 残りHP分の作業で撃破
    is_up, level_or_diff, event = study_logic.add_exp('programming', 1)
    assert event == 'BOSS_DEFEATED'
    data = study_logic.load_player_data()
    assert data['is_boss_active'] is False
