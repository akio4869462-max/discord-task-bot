...
活動の**累積分数を生データのまま持ち**、能力値は読み出すたびに導出する。
    ...
    能力 = 140 + 2.5 × (その能力に積んだ分)^0.70   （上限750）
...
# ⭕ 「配合も育成もかなり上手くやって20戦7勝（＝ちょうどG1に届く）」になるよう
#    シミュレーションで決めた。6.968 だった頃は週4時間でも15勝して無双していた。
#    普通(週4h) 4勝 ／ 良い(週9h) 5勝 ／ 最良(週14h＋最良の配合) 7勝。
GROWTH_K = 2.5
GROWTH_EXP = 0.70           # 逓減の効き。序盤ほど伸びやすい
ABILITY_CAP = 750           # G1平均(680)を少し超える水準。怪物でもG1で無双しない保険
...
# ⭕ G1種牡馬（9,750万円）が「良い世代の獲得賞金（約2億）の半分」になる値。収入は
