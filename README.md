...
* **過去問演習との自動連携**: `/exam log`で過去問演習を記録すると、問題数から算出した目安時間（1問1.5分）ぶんの「インプット」EXPが自動で加算されます。演習記録とRPGの二重入力が不要です。
...
* **曜日別メニュー表示**: `/training menu`で、曜日に応じたその日のトレーニングメニュー（種目・セット数×回数）と、該当する解説画像を表示します。就活RPGのEXP/レベルシステムとは独立して動作します。
