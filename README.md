...
### 5. 自宅トレーニング記録 (`training_logic.py`)
* **毎日同じメニューを表示**: `/training menu`で、ウォームアップ→サーキット→クールダウンの全身メニュー（種目・セット数×回数）を表示します。1セッションは30分相当の調教として、馬のスタミナとパワーに反映されます。解説画像は現在未整備（新しい種目に合ったものを準備中）。
* **完了ログ＆連続記録**: `/training log`でその日のトレーニングを完了として記録し、専用の連続記録（streak）を管理します。計画された休養日（月曜）を挟んでも連続記録は途切れません。
